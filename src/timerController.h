...
struct TimerEntry {
    ...
    uint8_t  minute;
    uint8_t  second;
    // "all_off","ring","spinner","rainbow","party","chase","clock","progress",
    // "wipe","random_yes_no","morse","weather_color","air_quality_color",
    // "bambu_mode_on"
    String   action;
    String   morseText;
    uint8_t  percent   = 0;   // progress action only
...
public:
    // ── Callbacks ─────────────────────────────────────────────────────────────
    std::function<void(uint8_t, uint8_t, uint8_t)> onRing;   // solid colour
    std::function<void(uint8_t, uint8_t, uint8_t)> onWipe;
    std::function<void()>                 onAllOff;
    std::function<void(bool)>             onSpinner;
    std::function<void(bool)>             onRainbow;
    std::function<void(bool)>             onParty;
    std::function<void(bool)>             onChase;
    std::function<void(bool)>             onClock;
    std::function<void(bool, uint8_t)>    onProgress;         // enabled + percent
    std::function<void()>                 onRandomYesNo;
    std::function<void(const String&)>    onMorse;
    std::function<void()>                 onWeatherColor;
    std::function<void()>                 onAirQualityColor;
...
private:
    ...
    void fromJson(JsonObject obj, TimerEntry& t) {
        ...
        t.percent   = obj["percent"]   | (uint8_t)0;
    ...
    void fire(TimerEntry& t) {
        ...
        else if (t.action == "ring"              && onRing)            onRing(t.ledR, t.ledG, t.ledB);
        else if (t.action == "wipe"              && onWipe)            onWipe(t.ledR, t.ledG, t.ledB);
        else if (t.action == "spinner"           && onSpinner)         onSpinner(true);
        else if (t.action == "rainbow"           && onRainbow)         onRainbow(true);
        else if (t.action == "party"             && onParty)           onParty(true);
        else if (t.action == "chase"             && onChase)           onChase(true);
        else if (t.action == "clock"             && onClock)           onClock(true);
        else if (t.action == "progress"          && onProgress)        onProgress(true, t.percent);
        ...
        else if (t.action == "air_quality_color" && onAirQualityColor) onAirQualityColor();
    ...
    void fireOff(TimerEntry& t) {
        ...
        if      (t.action == "spinner"       && onSpinner)   onSpinner(false);
        else if (t.action == "rainbow"       && onRainbow)   onRainbow(false);
        else if (t.action == "party"         && onParty)     onParty(false);
        else if (t.action == "chase"         && onChase)     onChase(false);
        else if (t.action == "clock"         && onClock)     onClock(false);
        else if (t.action == "progress"      && onProgress)  onProgress(false, 0);
