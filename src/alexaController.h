...
//
// Presents itself as a single Hue bridge with 5 devices:
//   - 1 "Extended color light" for the ring itself (full RGB)
//   - 4 "On/Off light" entries that toggle an effect each
//
// Protocol flow:
...
//   4. GET /api/{user}/lights      — light list (keys "1"–"5")
//   5. GET /api/{user}/lights/{k}  — individual light state
//   6. PUT /api/{user}/lights/{k}/state — colour / on-off control
//
// Light key → function mapping:
//   key "1" = Ringlight          → ring colour
//   key "2" = Ringlight spinner  → spinner effect on/off
//   key "3" = Ringlight rainbow  → rainbow effect on/off
//   key "4" = Ringlight party    → party effect on/off
//   key "5" = Ringlight chase    → theater chase on/off
//
...
class AlexaController {
    ...
    static const char* _name(uint8_t key) {
        static const char* n[] = { "", "Ringlight", "Ringlight spinner",
                                   "Ringlight rainbow", "Ringlight party", "Ringlight chase" };
        return (key >= 1 && key <= 5) ? n[key] : "";
    }

    // Current on/off state of the effect behind an effect key (2–5).
    bool _effectOn(uint8_t key) {
        switch (key) {
            case 2: return _ring->getEffect() == RingEffect::SPINNER;
            case 3: return _ring->getEffect() == RingEffect::RAINBOW;
            case 4: return _ring->getEffect() == RingEffect::PARTY;
            case 5: return _ring->getEffect() == RingEffect::CHASE;
            default: return false;
        }
    }

    // Turns one effect on or off, keeping its stored parameters.
    void _setEffect(uint8_t key, bool on) {
        switch (key) {
            case 2: _ring->setSpinner(on, _ring->getSpinnerR(), _ring->getSpinnerG(),
                                      _ring->getSpinnerB(), _ring->getSpinnerTail(),
                                      _ring->getSpinnerSpeed(), _ring->getSpinnerCW()); break;
            case 3: _ring->setRainbow(on, _ring->getRainbowCycleTime()); break;
            case 4: _ring->setParty(on, _ring->getPartyMadness()); break;
            case 5: _ring->setChase(on, _ring->getChaseR(), _ring->getChaseG(),
                                    _ring->getChaseB(), _ring->getChaseSpeed()); break;
            default: break;
        }
    }
    ...
    void _handleApi(AsyncWebServerRequest* req) {
            ...
            if (li >= 0) {
                ...
                if (key == 1) {
                    ...
                    if (!deserializeJson(doc, _body)) {
                        ...
                        Serial.printf("[Alexa] ring → %s r=%u g=%u b=%u\n",
                                      on ? "ON" : "OFF", r, g, b);
                        _ring->setSolid(r, g, b, on, false);
                        if (onChanged) onChanged();
                    }
                    _body = "";
                    req->send(200, "application/json", "[{\"success\":true}]");
        ...
        if (url.indexOf("/lights") > 0) {
            ...
            if (suffix.length() <= 1) {
                String resp = "{\"1\":" + _deviceJson(1);
                for (uint8_t k = 2; k <= 5; k++)
                    resp += ",\"" + String(k) + "\":" + _effectJson(k, _effectOn(k));
                resp += "}";
                req->send(200, "application/json", resp);
            } else {
                uint8_t key = suffix.substring(1).toInt();
                if (key == 1)
                    req->send(200, "application/json", _deviceJson(1));
                else if (key >= 2 && key <= 5)
                    req->send(200, "application/json", _effectJson(key, _effectOn(key)));
                else
                    req->send(200, "application/json", "{}");
            }
            return;
...
public:
    ...
    void begin(AsyncWebServer& server, RingController& ring) {
        _ring = &ring;
        ...
        server.on("/desc.xml", HTTP_GET, [this](AsyncWebServerRequest* req) {
            ...
            snprintf(buf, sizeof(buf),
                ...
                "<friendlyName>Ringlight (%s:80)</friendlyName>"
        ...
        Serial.println("[Alexa] Hue bridge ready (ring + 4 effects)");
    }
